                    ...
                    <div class="row" style="margin: 31px;box-shadow: 1px 0px 16px 0px #88888863;padding: 25px;">
                        ...
                        @if($notice->image != null)
                            <div class="col-md-12" style="display: flex; justify-content: center">
                                @if(pathinfo(storage_path('uploads/download'.$notice->image))['extension'] == "jpg")
                                    <a href="{{asset(('uploads/download/'.$notice->image))}}">
                                        <img id="myImg" src="{{asset(('uploads/download/'.$notice->image))}}"
                                             height="200px"
                                             width="200px"></a>
                                    <a style="margin-left: 10px"
                                       href="{{route('file.download',$notice->image)}}"
                                       class="btn btn-dark btn-sm" download="">Download</a>
                                @endif
                                @if(pathinfo(storage_path('uploads/download'.$notice->image))['extension'] == "pdf")
                                    <a href="{{asset(('uploads/download/'.$notice->image))}}" target="_blank"
                                       style="font-size: 25px;">{{$notice->title}}.{{pathinfo(storage_path('uploads/download'.$notice->image))['extension']}} </a>
                                    <div class="" style="margin-left: 18px">
                                        <a class="btn btn-dark btn-sm"
                                           href="{{asset(('uploads/download/'.$notice->image))}}"
