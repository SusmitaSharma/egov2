...
<div class="box">
               ...
               <div class="box-tools pull-right">
                        <a href="{{route('slider.create')}}" class="btn btn-success btn-new" data-title="नयाँ बनाउने" data-col-width="Medium"><i class="fa fa-plus"></i></a>
                  ...
                  <table id="example3" class="table table-bordered table-striped">
                    ...
                    @foreach($sliders as $k=>$v)
                      <tr>
                        ...
                        <td>
                          <a href="{{route('slider.edit',$v)}}" class="btn-edit btn-xs btn btn-primary" data-title="सम्पादन : नेपाल सरकार"><i class="fa fa-edit"></i></a>
                    <a href="javascript:void(0)"  onClick="if(confirm('Are you sure want to delete?'))
                          {document.getElementById('delete-form-{{$v->id}}').submit();}else{event.preventDefault();}"  title="Delete" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i></a>
                           <form id="delete-form-{{$v->id}}" action="{{route('slider.destroy',$v->id)}}" method="POST">
                                {{csrf_field()}}
                                {{method_field('DELETE')}}
