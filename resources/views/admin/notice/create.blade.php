...
@section('content')
    ...
    <section class="content">
        ...
        <form action="{{$edit? route('notice.update',$notice->id):route('notice.store')}}" class="form-horizontal"  method="post" id="userForm" enctype="multipart/form-data">
            @csrf
            @if($edit)
                @method('PUT')
            @endif
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            ...
                            <i class="fa fa-file"></i>
                                        ...
                                        <input class="form-control" id="Title" name="title" type="text" value="{{$edit?$notice->title:old('title')}}" placeholder="शिर्षक">
                                    </div>
                                </div>
                            </div>
                            <br>
                            <div class="row">
                                ...
                                <div class="col-md-9">
                                    <div class="input-group">
                                        ...
                                        <textarea class="form-control bsb-tinymce" name="description">{{$edit?$notice->description:old('description')}}</textarea>
                                    </div>
                                </div>
                            ...
                            <i class="fa fa-dashcube"></i>
                                        ...
                                        <input class="form-control datepicker" id="publishedDate" name="publishedDate" type="text"
                                               value="{{$edit?$notice->published_date:old('published_date')}}">
                                    </div>
                                </div>
                            </div>
                            <br>
                            ...
                            <div class="row">
                                ...
                                <i class="fa fa-file-o"></i>
                                            ...
                                            <select name="status" class="form-control">
                                                <option value="1" @if($edit){{(($notice->status=='1')? 'selected' : '')}} @endif>Active</option>
                                                <option value="0" @if($edit){{(($notice->status=='0')? 'selected' : '')}} @endif>Inactive</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="fileupload fileupload-exists" data-provides="fileupload" data-name="myimage">
                                ...
                                <div class="fileupload-new thumbnail" style="width: 200px; height: 150px;">
                                    ...
                                    @else
                                        ...
                                        @if($notice->isImage())
                                            <img src="{{$notice->image()}}" style="width: 150px" />
                                        @else
                                            @if(!is_null($notice->image))
                                                <a href="{{route('notice.download',$notice->image)}}">{{$notice->image}}(Click To Download)</a>
                                            @endif
                                        @endif
